...
const Dashboard = () => {
  ...
  // Mark component as visible after initial render (prevents premature loading overlay)
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    ...
    // 1. Component is visible (not behind auth screen)
    // 2. Loading is complete (!loading)
    // 3. Data exists (data.length > 0)
    // 4. Haven't auto-generated yet (!hasAutoGenerated.current)
    if (isComponentVisible && !loading && data.length > 0 && !hasAutoGenerated.current) {
  ...
  }, [isComponentVisible, loading, data.length]); // Only depend on loading and data.length to prevent double-firing
