...
const Dashboard = () => {
  ...
  React.useEffect(() => {
    // Only auto-generate once when data becomes available AND has text responses
    if (data.length > 0 && !hasAutoGeneratedInsights.current) {
  ...
  }, [data.length, data]); // Run when data becomes available (refreshInsights excluded to prevent re-runs)
