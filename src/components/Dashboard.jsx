...
const Dashboard = () => {
  ...
  // Auto-generate insights when data first loads
  const hasAutoGeneratedInsights = React.useRef(false);

  React.useEffect(() => {
    // Only auto-generate once when:
    // 1. Loading is complete (!loading)
    // 2. Data exists (data.length > 0)
    // 3. Haven't auto-generated yet (!hasAutoGeneratedInsights.current)
    // 4. At least one player has text responses
    if (!loading && data.length > 0 && !hasAutoGeneratedInsights.current) {
      // Check if at least one player has text responses
      ...
      if (hasTextResponses) {
        hasAutoGeneratedInsights.current = true;

        // Add a small delay to ensure all data is fully loaded and stable
        const timer = setTimeout(() => {
          console.log('Auto-generating insights with', data.length, 'players');
          refreshInsights();
        }, 500);
  ...
  }, [loading, data.length, data]); // Run when loading completes and data is available
